...
[自動フォーム変換サービス（AFCS）](/help/using/introduction.md)では、以下のソースフォームをアダプティブフォームに変換します。
...
AcroForm または XFA ベースの PDF フォームを自動フォーム変換サービス（AFCS）のソースフォームとして使用している場合は、次のことができます。
