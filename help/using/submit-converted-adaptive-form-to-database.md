...
PDFファイルは、Automated forms conversionサービス（AFCS）への入力として機能します。 自動フォーム変換サービスを実行すると、このファイルがアダプティブフォームに変換されます。 以下の画像は、PDF 形式のサンプルの「Contact Us」フォームを示しています。
...
の使用 [変換するAutomated forms conversionサービス（AFCS）](convert-existing-forms-to-adaptive-forms.md) この [お問い合わせフォーム](#sample-adaptive-form) からデータ連結を使用するアダプティブフォームへ。 アダプティブフォームを生成する際に、「**[!UICONTROL データバインディングがないアダプティブフォームを生成]**」チェックボックスが無効になっていることを確認してください。
