...
* 複雑な動的レイアウトのPDF forms、点線の輪郭を持つフィールド、または入力済みのフィールドはサポートされていません。
...
* 枠なしテーブル、階層化テーブル、プレースホルダー値を持つテーブルなどの複雑なテーブルはサポートされていません。 こうした複雑なテーブルの追加や変更を行う場合は、変換処理の完了後にアダプティブフォームエディターを使用してください。 変換サービスでは、単純なテーブル（フィールド値が設定されておらず、ヘッダーが適切に配置され、境界線が明確なテーブル）を使用してください。
