...
変換サービスでサポートされているのは、ブランクのフォームとフィールドに値が設定されていないフォームだけです。 そのため、既に値が設定されているフォームや、個人情報が含まれているフォームはアップロードしないでください。 また、ソースフォーム内の事前入力済みデータ、個人情報(PII)、機密情報、および所有者情報も削除します。 <br/>
